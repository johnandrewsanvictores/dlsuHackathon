    ...
    <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <p className="font-roboto text-sm text-slate-600 mb-3">
            {job.location}
          </p>
        </div>
        {job.matchScore && (
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <span
          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getWorkArrangementColor(
            job.workArrangement
          )}`}
        >
...
const ApplyJobModal = ({ isOpen, job, onClose, onApply }) => {
  ...
  const checkResumeStatus = async () => {
    try {
      ...
      const hasResume =
        response.data.resumeContext && response.data.resumeContext.length > 0;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        setEnhancedResume({
          ...
          skills: [
            "Technical skills from job posting",
          ...
          <div className="space-y-6">
            ...
            {showEnhancement && enhancedResume && (
              <div className="bg-brand-honey-50 border border-brand-honey-200 rounded-lg p-6">
                <h3 className="font-medium text-brand-bee mb-4">
                  🤖 AI Resume Enhancement Tips
                </h3>
    ...
    try {
      ...
      if (filters.searchWithResume) {
        ...
        if (filters.workArrangement !== "all")
          additionalFilters.workArrangement = filters.workArrangement;
        if (filters.employmentType !== "all")
          additionalFilters.employmentType = filters.employmentType;
        if (filters.experienceLevel !== "all")
          additionalFilters.experienceLevel = filters.experienceLevel;
        if (filters.industry !== "all")
          additionalFilters.industry = filters.industry;
        if (filters.location) additionalFilters.location = filters.location;
        if (filters.salaryMin)
          additionalFilters.salaryMin = parseInt(filters.salaryMin);
        if (filters.salaryMax)
          additionalFilters.salaryMax = parseInt(filters.salaryMax);

        console.log("Sending resume filter request:", {
          additionalFilters,
                    ...
                    <div className="bg-brand-honey-50 border border-brand-honey-200 rounded-lg p-3">
                      <p className="text-sm text-brand-bee">
                        <span className="font-medium">
                          Resume Search Active:
                        </span>{" "}
                        Jobs will be matched based on your uploaded resume
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleFilterChange("workArrangement", e.target.value)
                      }
                      className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-honey focus:border-brand-honey"
                    >
